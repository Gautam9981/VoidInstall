        ...
        if method == "Launch cfdisk (Recommended)":
            ...
            try:
                if VOID_SYSTEM:
                    ...
                    # In test mode, just check if cfdisk exists
                    import shutil
                    if not shutil.which('cfdisk'):
                        npyscreen.notify_confirm("cfdisk not found! In a real installation, it would be automatically installed.", title="Test Mode")
...
class InstallProgressForm(npyscreen.ActionForm):
    def create(self):
        ...
        self.progress = self.add(npyscreen.FixedText, 
                                value="Ready to start installation. Press OK to begin or Cancel to go back.",
                                max_height=6,
                                editable=False)
